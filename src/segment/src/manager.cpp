...
{
	string integrate_left_right = "left_and_right";
	if (param->pub_ground != "")
	{
	...
	pub_left_right = nh.advertise<sensor_msgs::PointCloud2>(integrate_left_right, 10); //integrate_left_right
...
void Manager::doRslidarWork(const sensor_msgs::PointCloud2ConstPtr &in_cloud_ptr,const std::string topicName)
{
	...
	for (size_t i = 0; i < cache_livox_point_cloud.size() ; i++)
	{
		...
		*cloud_sum_ptr += *cloud_ptr;
		publish_cloud(pub_left_right, cloud_sum_ptr);
	}
